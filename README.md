...
Чтобы заполнить поля API_HASH и API_ID, необходимо перейти по ссылке: 

```bash
docker compose up -d telegram-parser
```
На данной странице необходимо авторизоваться под своим телеграмм аккаунтом и нажать на `API development tools`. Там будут лежать указанные поля.
...
Чтобы запустить парсер через `TELEGRAM_PHONE`:
1. Указать в .env `TELEGRAM_PHONE` - номер телефона, к которому привязан аккаунт
2. Запустить парсер. После запуска в телеграмм должно придти уведомление, содержащее `TELEGRAM_CODE`. Заполните соответствующее поле в .env
3. После этого необходимо еще раз запустить парсер. Если на аккаунте установлена двухфакторная авторизация, необходимо также заполнить поле `TELEGRAM_PASSWORD` в .env

Иначе говоря, если контейнер пишет, что код отправлен и требуется `TELEGRAM_CODE`, порядок действий такой:
